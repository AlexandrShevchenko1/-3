.include "macros.s"
.global replace, er_name, process_data, results_to_console
...
process_data:                                 # searches for a sequence
	...
	found_sequence:
	    ...
	    print_string("Input path to output file: ")
	    read_string(output_file_name, NAME_SIZE)
	    ...
	    mv a6 t2
	    jal results_to_console
	    j finish
	       ###############################################################
	finish_search:
	   j external_loop
	stop_program_execution:
	   print_string("No sequence found\n")
           ###############################################################
	finish:   
	    close_file(a2)
	    lw ra (sp)
